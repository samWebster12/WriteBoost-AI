import {
  ...
  export function ProductDescriptionEditor() {
    ...
    const {
        ...
        url: "/apps/api/backend/2023-04/products.json",
        reactQueryOptions: {
          onSuccess: (newData) => {
            ...
            setDescriptionFieldValue(retrievedOptions[0].description ? retrievedOptions[0].description : '');
    ...
    const handleSelectChange = (value) => { 
        ...
        setDescriptionFieldValue(option.description ? option.description : '')
      };
    ...
    const handleNewDescription = useCallback(async () => {
        ...
        message += ' that is around 20 words long';
        console.log(message);

        try {
          console.log('trying to get chatgpt')
          const response = await fetch('/apps/api/backend/chatgpt', {
            method: 'POST',
            headers: {
    ...
    const handleUpdateDescription = useCallback(async () => {
        ...
        try {
            const response = await fetch('/apps/api/backend/2023-04/products/' + selectedOption.id + '.json', { 
                method: 'PUT',
                headers: {
                    "Content-Type": "application/json",
                },
